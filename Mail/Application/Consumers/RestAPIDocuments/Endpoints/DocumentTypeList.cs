...
namespace Application.Consumers.RestAPIDocuments.Endpoints
{
    ...
    {
        public async static Task<List<DocumentType>> CallRestApi(string owner_id)
        {
            string apiUrl = "http://localhost:5046/documenttype/list_types";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    foreach (var result in results)
                    {
                        ...
                        DocumentType tag = new DocumentType
                        {
